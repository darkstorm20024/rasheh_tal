...
async function handleStats(req, res) {
  const [
    { count: totalClients }, { count: totalCandidates }, { count: totalRequests },
    { count: pendingRequests }, { count: approvedRequests }, { count: totalPlacements },
    { count: totalArticles }, { count: totalTestimonials }
  ] = await Promise.all([
  ...
  return reply(res, 200, {
    ok: true,
    stats: {
      totalClients: totalClients || 0, totalCandidates: totalCandidates || 0, totalRequests: totalRequests || 0,
      pendingRequests: pendingRequests || 0, approvedRequests: approvedRequests || 0, totalPlacements: totalPlacements || 0,
      totalArticles: totalArticles || 0, totalTestimonials: totalTestimonials || 0
    }
  });
}
...
async function handleRequests(req, res) {
  ...
  if (req.method === 'PUT') {
    ...
    const { data, error } = await db().from('cv_requests').update({ status: body.status, notes: body.notes || null }).eq('id', body.id).select().single();
    if (error) throw error;
...
async function handleSettings(req, res) {
  ...
  if (req.method === 'PUT') {
    const body = readBody(req);
    const updates = {
      id: 1, hero_title: body.hero_title, hero_text: body.hero_text, video_url: body.video_url || '',
      video_title: body.video_title || '', video_text: body.video_text || '', socials: body.socials || {},
      bank: body.bank || {}, news: body.news || []
    };
...
async function handleArticles(req, res) {
  ...
  if (req.method === 'POST') {
    ...
    const payload = {
      slug, title: body.title, excerpt: body.excerpt || '', content: body.content, cover_image: body.cover_image || '',
      author_name: body.author_name || 'فريق رَشّح', published: Boolean(body.published),
      published_at: body.published ? new Date().toISOString() : null, tags: body.tags || [],
      seo_title: body.seo_title || body.title, seo_description: body.seo_description || body.excerpt || ''
    };
  ...
  if (req.method === 'PUT') {
    ...
    const { data, error } = await db().from('articles').update(updates).eq('id', body.id).select().single();
    if (error) throw error;
    return reply(res, 200, { ok: true, article: data });
  }
...
async function handleTestimonials(req, res) {
  ...
  if (req.method === 'POST') {
    ...
    const payload = {
      author_name: body.author_name, author_role: body.author_role || '', company_name: body.company_name || '',
      content: body.content, rating: body.rating || 5, avatar_url: body.avatar_url || '', published: Boolean(body.published)
    };
  ...
  if (req.method === 'PUT') {
    ...
    const updates = { ...body };
    delete updates.id;
    const { data, error } = await db().from('testimonials').update(updates).eq('id', body.id).select().single();
    if (error) throw error;
    return reply(res, 200, { ok: true, testimonial: data });
  }
  if (req.method === 'DELETE') {
...
async function handlePlans(req, res) {
  ...
  if (req.method === 'PUT') {
    ...
    const updates = { ...body };
    delete updates.id;
    const { data, error } = await db().from('plans').update(updates).eq('id', body.id).select().single();
    if (error) throw error;
