...
async function handleLogin(req, res) {
  ...
  return reply(res, 200, { ok: true, access_token: data.session.access_token, user: { id: data.user.id, email: data.user.email } });
}
...
async function handleRegister(req, res) {
  ...
  if (password.length < 8) return reply(res, 400, { detail: 'كلمة المرور يجب أن تكون 8 أحرف على الأقل' });
  if (email.toLowerCase() === ADMIN_EMAIL.toLowerCase()) return reply(res, 400, { detail: 'هذا البريد محجوز' });

  const supa = db();
  const { data: authData, error: authError } = await supa.auth.admin.createUser({
    email, password, email_confirm: true, user_metadata: { role: 'company' }
  });
  if (authError) throw new Error(authError.message || 'فشل إنشاء الحساب');

  const { error: profileError } = await supa.from('clients').insert({
    auth_user_id: authData.user.id, company_name, contact_person, city, email, phone, plan: 'basic'
  });
