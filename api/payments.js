  ...
  const orderResp = await fetch(`${base}/v2/checkout/orders`, {
    ...
    body: JSON.stringify({
      ...
      purchase_units: [{
        ...
        amount: { currency_code: 'USD', value: (Number(plan.price) / 3.75).toFixed(2) },
        description: `باقة ${plan.name} — رَشّح`
      }],
      application_context: {
        return_url: `${baseUrl}/api/payments?resource=paypal-capture&payment_id=${payment.id}`,
        cancel_url: `${baseUrl}/billing.html?status=cancelled`
      }
    })
  });
...
// ---------- Admin: manage pending bank transfers ----------
