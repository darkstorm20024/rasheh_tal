...
async function handleStats(req, res) {
  ...
  return reply(res, 200, { ok: true, stats: { totalClients: totalClients || 0, totalCandidates: totalCandidates || 0, totalPlacements: totalPlacements || 0, avgTimeToHire: '5 أيام' } });
}
...
async function handleCandidateRequest(req, res) {
  ...
  const { data: existing } = await db()
    .from('cv_requests').select('id, status').eq('client_id', client.id).eq('candidate_id', candidateId).maybeSingle();
  ...
  const { data: reqRow, error: insErr } = await db().from('cv_requests').insert({
    client_id: client.id, candidate_id: candidate.id, full_name: candidate.full_name, email: candidate.email,
    phone: candidate.phone, city: candidate.city, experience_years: candidate.experience_years,
    skills: candidate.skills, cv_url: candidate.cv_url, status: 'pending'
  }).select().single();
