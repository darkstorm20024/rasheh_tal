...
module.exports = async (req, res) => {
  try {
    ...
    if (error || !data) {
      return reply(res, 200, { ok: true, user: { id: currentUser.id, email: currentUser.email }, profile: null });
    }

    return reply(res, 200, { ok: true, user: { id: currentUser.id, email: currentUser.email }, profile: { ...data, is_admin: false } });
  } catch (error) {
